...
Operator-relevant summary of the July 2026 audit-remediation series (Critical/High → Medium → Low severity passes, plus the follow-on shared rate limiter). See `CLAUDE.md` for full technical detail and file references.

- **Admin-tool registration gating** — `pfw_manage_users` is registered only when `PFW_ENABLE_USER_MANAGEMENT=true` (default off); absent from `tools/list` entirely in STDIO and in HTTP deployments that don't opt in.
- **Fail-fast scope verification** — startup refuses to run if the `pfw:admin` scope gate fails to attach to the registered admin tool, instead of silently serving it ungated.
- **Query validation** — free-form search query parameters are capped at 1000 characters before being sent upstream.
- **Security-log content minimization** — the security logger is the sole owner of its sanitizing filter (masked emails/IPs, no raw tokens or URLs); control characters are stripped before masking to block log-line forgery.
- **Refresh-token family revocation** — replay of a spent/rotated OAuth refresh token revokes every live token for that (client, identity) pair, not just the one token.
- **Recent-downloads endpoint locked down** — `GET /api/recent-downloads` now requires `X-Proxy-Token`, closing a path that leaked working 7-day persistent-link credentials with no auth.
- **SSRF-hardened registration downloads** — cross-MCP `download_url` values are validated by parsed hostname (`https` + `uspto.gov`/`*.uspto.gov`) at registration and re-validated at both streaming sinks before the real USPTO API key is attached.
- **OS-managed at-rest key storage** — Linux keys are now `systemd-creds`-encrypted (user+machine bound, TPM2-sealed where available), matching the existing Windows DPAPI posture; a unified Fernet key store replaced three drifting copies.
- **Upstream PDF integrity check** — downloads verify the `%PDF-` magic bytes before response headers are sent, instead of after a partial stream.
- **Streaming request-size cap hardened** — the 1MB body-size limit is enforced as a running byte count, closing a chunked-transfer-encoding bypass.
- **Reverse-proxy IP attribution** — `PROXY_TRUSTED_IPS` controls which upstream peers' `X-Forwarded-For` is trusted for client-IP-based rate limiting and audit logging (default `127.0.0.1` only).
- **Non-root container + XXE hardening** — the Docker image runs as a non-root user; XML parsing goes through `defusedxml`.
- **Shared cross-process USPTO rate limiter** — see the dedicated README section below; keeps all 4 USPTO MCPs under one API key's rate/burst limits when co-located.
