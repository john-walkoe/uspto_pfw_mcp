...
# secure-storage keys mid-test — only run it deliberately.
...
│       ├── tools/                 # MCP tool registration (search, documents, OA, guidance, admin)
│       ├── auth/                  # Optional OAuth 2.1 sign-in (PFW_AUTH_MODE=oauth)
│       ├── ui/                    # MCP App HTML views (ui://pfw/ resources)
│       ├── shared/
...
Retrieved prosecution text is served verbatim — nothing is stripped or
...
- **`injection_scan`** — a detection-only runtime scanner
  ...
  naming the flagged document and the kinds of pattern found — never the
...
`PFW_ENABLE_USER_MANAGEMENT=true` (default off — in STDIO and non-OAuth HTTP
