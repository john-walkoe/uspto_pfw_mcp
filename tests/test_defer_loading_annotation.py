...
Three tools are eager (`PFW_search_applications_minimal`, `PFW_get_guidance`,
`PFW_get_application_documents`); the rest are deferred, and that split is the
...
from patent_filewrapper_mcp.fastmcp_compat import apply as apply_compat
...
# cannot quietly change which tools load eagerly. Matches the three tools
# SERVER_INSTRUCTIONS advertises as ALWAYS-AVAILABLE.
EAGER = {
...
async def test_tool_titles_are_pinned_to_tool_names():
    """FastMCP 4 derives a title from the name when none is set
    ("PFW Get Guidance"); `_pin_tool_titles` keeps the displayed label equal to
