...
# DESTRUCTIVE: test_key_storage_retrieval() overwrites the REAL DPAPI-stored
# USPTO/Mistral keys. Opt in deliberately with PFW_RUN_KEY_TESTS=1.
if not os.getenv("PFW_RUN_KEY_TESTS"):
    pytest.skip(
        "Destructive key-management test skipped (overwrites real stored keys). "
        "Set PFW_RUN_KEY_TESTS=1 to run deliberately.",
...
def test_mcp_integration():
    ...
    try:
        ...
        # Try Enriched Citations
        try:
            from uspto_enriched_citation_mcp.api.client import EnrichedCitationClient  # noqa: F401
            from uspto_enriched_citation_mcp.config.settings import Settings  # noqa: F401
