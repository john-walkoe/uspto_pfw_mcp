...
async def test_load_access_token_rejects_garbage_and_internal() -> None:
    ...
    # The plain internal token grants pfw:user ONLY — admin requires the
...
async def test_internal_admin_token_grants_admin_scope() -> None:
    """A separate internal admin token grants pfw:user + pfw:admin;
    the plain internal token still only grants pfw:user."""
...
async def test_admin_tools_scope_gated() -> None:
    ...
    @mcp.tool(name="PFW_search_applications_minimal")
    ...
    assert not tools["PFW_search_applications_minimal"].auth
...
async def test_mode_none_stack_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async with httpx.AsyncClient(
        ...
        # Probe shim still outermost: /mcp with an Accept the transport would
        # refuse → 401 even with a valid key (claude.ai format-probe
        # interception). Accept is stated explicitly because httpx defaults it
        # to `*/*`, which BOTH protocol eras accept — the shim must let that
        # through, and pass-through is covered in
        # tests/test_probe_middleware_accept.py (it needs a stub inner app,
        # since the real one wants a lifespan ASGITransport does not run).
...
async def test_allowed_redirect_hosts_can_be_extended(
    ...
    monkeypatch.setenv("PFW_AUTH_ALLOWED_REDIRECT_HOSTS", "partner.example")
    provider, _ = make_provider()
    client = OAuthClientInformationFull(
...
async def test_open_registration_opt_in_restores_the_old_behavior(
    ...
    monkeypatch.setenv("PFW_AUTH_OPEN_REGISTRATION", "true")
    provider, _ = make_provider()
    client = OAuthClientInformationFull(
