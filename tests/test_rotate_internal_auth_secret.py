...
@pytest.fixture
def rotate_module():
    # The script inserts src/ onto sys.path at import time; harmless to redo.
    return _load_script()
...
class TestRotateScript:
    ...
    def test_writes_new_comma_old_and_prints_both_steps(
        ...
        monkeypatch.setattr(
            "patent_filewrapper_mcp.shared_secure_storage.store_internal_auth_secret",
            _fake_store,
        )
    ...
    def test_a_comma_separated_existing_secret_rotates_only_the_current_one(
        ...
        """An already-comma-listed value (mid-rotation) keeps only its
        current root as the new "previous" -- the retired one is dropped,
