...
or more text remains)::
    ...
    "_window": {
        "unit": "page",                # "page" when the window edges were
...
counts, in both units, so ``next_offset`` can be passed straight back into
the caller's offset parameter. ``unit`` reports only whether the window
edges were snapped to page boundaries.
...
def window_text(
    ...
    *,
    ...
    (``unit: "page"``) so a page is never split mid-way; otherwise the window
    is a raw character slice (``unit: "char"``). A page larger than the whole
