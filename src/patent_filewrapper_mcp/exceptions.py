...
class RequestTimeoutError(PatentFileWrapperError):
    """Request timeout errors.

    Named RequestTimeoutError (not TimeoutError) so it can never shadow
    builtins.TimeoutError in an `except` chain (audit F20).
    """
    def __init__(self, message: str, request_id: Optional[str] = None):
