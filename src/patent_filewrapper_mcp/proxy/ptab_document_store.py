...
def _cipher() -> Fernet:
    ...
    if _fernet_cipher is None:
        _fernet_cipher = get_or_create_fernet("PTAB_DOCSTORE_KEY", ".ptab_docstore_key")
...
def _sanitize_filename(raw: Optional[str]) -> Optional[str]:
    ...
    if not raw:
        return raw
    # Remove all control characters including CR, LF, tab, null, etc.
...
class PTABDocumentStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        Args:
            db_path: Path to SQLite database (default: ptab_documents.db in
                the hardened data dir — audit L14)
        """
        if db_path:
            self.db_path = db_path
        else:
            self.db_path = str(migrate_data_file("ptab_documents.db"))
    ...
    def cleanup_expired_documents(self, max_age_days: int = 7) -> int:
        ...
        try:
            ...
            try:
                cursor = conn.execute(
                    "DELETE FROM ptab_documents WHERE registered_at < datetime('now', ?)",
                ...
                conn.close()
            if deleted > 0:
                logger.info(f"Cleaned up {deleted} expired PTAB document registration(s)")
            return deleted
        except Exception as e:
            logger.error(f"Error during PTAB document cleanup: {e}")
            return 0
