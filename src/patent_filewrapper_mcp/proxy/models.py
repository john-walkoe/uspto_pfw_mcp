...
def _validate_uspto_download_url(v: str) -> str:
    """Shared download_url validator: https + a real uspto.gov hostname.

    Hostname is parsed, not substring-matched (audit H2 SSRF fix):
    'https://evil.example/uspto.gov' and 'https://uspto.gov.evil.com/...'
    must both be rejected — the proxy attaches the real USPTO API key when
    fetching this URL.
    """
    parsed = urlparse(v)
    ...
    if host != 'uspto.gov' and not host.endswith('.uspto.gov'):
...
class FPDDocumentRegistration(BaseModel):
    ...
    def validate_download_url(cls, v: str) -> str:
        ...
        return _validate_uspto_download_url(v)
