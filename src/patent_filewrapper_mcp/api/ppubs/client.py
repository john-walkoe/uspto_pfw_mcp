...
class PpubsClient:
    ...
    async def get_session(self):
        ...
        if response.status_code != 200:
            ...
            logger.error(f"Failed to establish session: {response.status_code}")
            return None

        logger.debug(f"Session response received ({len(response.text)} chars)")
    ...
    async def make_request(self, method, url, **kwargs):
        ...
        try:
            ...
            # Flow metadata only — response bodies (patent/search content)
            # stay out of logs
            logger.debug(f"Response for {method}: {response.status_code} ({len(response.text)} chars)")
    ...
    async def run_query(
        ...
        logger.info(f"Running Public Search query ({len(query)} chars)")
        ...
        # Counts only — result content stays out of logs
        logger.debug(
            f"Search returned {result.get('numFound', 'unknown')} total, "
            f"{len(result.get('patents', []) or [])} in page"
        )
    ...
    async def get_document(self, guid, source_type) -> Dict[str, Any]:
        ...
        # Size only — document content stays out of logs
        document_data = response.json()
        logger.debug(f"Document data received ({len(response.text)} chars)")
