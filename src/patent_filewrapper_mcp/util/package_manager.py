...
class PackageManager:
    ...
    def _create_package_document(self, doc_data: Dict[str, Any], category: str) -> PackageDocument:
        ...
        _proxy_base = os.getenv("PFW_PROXY_BASE_URL", f"http://localhost:{self.proxy_port}")
        proxy_url = f"{_proxy_base}/download/{doc_data.get('applicationNumber', '')}/{doc_id}" if doc_id else None
