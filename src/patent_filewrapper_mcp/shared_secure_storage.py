...
- systemd-creds encryption on Linux: user+machine-bound, TPM2-sealed when
  available (util/linux_secret_store.py); legacy plaintext files migrate to
  encrypted form on first read. Plaintext 0600 file only where neither exists
  (e.g. Docker, macOS)
...
from .util.dpapi_utils import encrypt_with_dpapi as _encrypt_dpapi
from .util.dpapi_utils import decrypt_with_dpapi as _decrypt_dpapi
...
def _encrypt_with_dpapi(data: bytes, entropy: bytes) -> bytes:
    ...
    Delegates to the shared encrypt_with_dpapi implementation in dpapi_utils.
    Kept as local wrapper for internal API compatibility.
    """
...
def _decrypt_with_dpapi(encrypted_data: bytes, entropy: bytes) -> bytes:
    ...
    Delegates to the shared decrypt_with_dpapi implementation in dpapi_utils.
    Kept as local wrapper for internal API compatibility.
    """
    ...
    def _store_single_key(self, key: str, path: Path, key_name: str) -> bool:
        ...
        try:
            if sys.platform == "win32":
                ...
                # Non-Windows: OS-managed encryption via systemd-creds --user
                # (user+machine-bound, TPM2-sealed when available — the DPAPI
                # analog, audit M1). Plaintext 0600 file only as last resort.
                from .util.linux_secret_store import encrypt_to_file_bytes

                file_bytes, encrypted = encrypt_to_file_bytes(
                    key.encode('utf-8'), key_name
                )
                ...
                if encrypted:
                    logger.info(
                        f"Stored {key_name} via systemd-creds (user+machine bound) at: {path}"
                    )
                else:
                    logger.warning(
                        f"OS-managed encryption unavailable - stored {key_name} "
                        f"as plaintext 0600 file at: {path}"
                    )
                return True
    ...
    def _load_single_key(self, path: Path, key_name: str) -> Optional[str]:
        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                # Non-Windows: systemd-creds encrypted blob, or legacy plaintext
                from .util.linux_secret_store import (
                    backend_available,
                    decrypt_from_file_bytes,
                )

                plaintext, was_encrypted = decrypt_from_file_bytes(
                    path.read_bytes(), key_name
                )
                if plaintext is None:
                    logger.error(
                        f"Failed to decrypt {key_name} (wrong user/machine or "
                        f"corrupted file): {path}"
                    )
                    return None
                key = plaintext.decode('utf-8').strip()
                if not was_encrypted and backend_available():
                    # One-time migration: re-store legacy plaintext through the
                    # encrypted backend (audit M1)
                    if self._store_single_key(key, path, key_name):
                        logger.info(f"Migrated {key_name} to systemd-creds encrypted storage")
                logger.debug(f"Loaded {key_name} from: {path}")
                return key
...
def store_secure_api_key(key: str, key_name: str) -> bool:
    ...
    if key_name == "USPTO_API_KEY":
        ...
        # Delegate to generic storage for arbitrary key names
        return store_generic_secret(key, key_name)


def get_generic_secret(key_name: str) -> Optional[str]:
    """
    Retrieve an arbitrary secret using DPAPI protection on Windows.

    On Windows: uses DPAPI with per-user, per-machine encryption (CWE-330 compliant).
    On non-Windows: falls back to file permissions.

    File path: ~/.uspto_generic_secret_{key_name}

    Args:
        key_name: Unique identifier for the secret (e.g. "PROXY_ENCRYPTION_KEY")

    Returns:
        Decrypted secret string, or None if not found or decryption fails
    """
    try:
        storage = UnifiedSecureStorage()
        return storage._load_single_key(
            storage.home_dir / f".uspto_generic_secret_{key_name}",
            key_name
        )
    except Exception as e:
        logger.error(f"Failed to load generic secret '{key_name}': {e}")
        return None


def store_generic_secret(secret: str, key_name: str) -> bool:
    """
    Store an arbitrary secret with DPAPI protection on Windows.
    ...
    try:
        storage = UnifiedSecureStorage()
        return storage._store_single_key(
            secret,
            storage.home_dir / f".uspto_generic_secret_{key_name}",
            key_name
        )
    except Exception as e:
        logger.error(f"Failed to store generic secret '{key_name}': {e}")
        return False
